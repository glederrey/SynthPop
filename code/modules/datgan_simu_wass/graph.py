...
from tensorpack import BatchNorm, LayerNorm, Dropout, FullyConnected, InputDesc, ModelDescBase
...
class GraphBuilder(ModelDescBase):
    ...
    def __init__(
        ...
        self.lambda_ = 10  # Param for WGAN GP loss
    ...
    def build_losses(self, vecs_real, vecs_fake):
        ...
        Args:
            discrim_real (tensorflow.Tensor): discrim logits from real samples.
            discrim_fake (tensorflow.Tensor): discrim logits from fake samples from generator.
        ...
        with tf.name_scope("GAN_loss"):
            ...
            with tf.name_scope("discrim"):
                wgan_d_loss = -tf.reduce_mean(d_logit_real) + tf.reduce_mean(d_logit_fake)

                gp_loss = self.gradient_penalty(vecs_real, vecs_fake)

                d_loss = wgan_d_loss + self.lambda_*gp_loss + \
                    tf.contrib.layers.apply_regularization(
                        tf.contrib.layers.l2_regularizer(self.l2norm),
                        tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, "discrim"))

                wgan_d_loss = tf.identity(wgan_d_loss, name='wgan_d_loss')
                gp_loss = tf.identity(gp_loss, name='gp_loss')
                self.d_loss = tf.identity(d_loss, name='discr_loss')
            ...
            with tf.name_scope("gen"):
                ...
                self.g_loss = tf.identity(g_loss, name='g_loss')

            add_moving_summary(self.g_loss, wgan_d_loss, gp_loss, self.d_loss, decay=0.)
    ...
    def gradient_penalty(self, real_data, fake_data):
        ...
        for f, r in zip(fake_data, real_data):
            interpolates.append(alpha*r + (1-alpha)*f)

        # Gradient of discriminator on the interpolate
