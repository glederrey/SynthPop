...
from modules.datgan_postLSTM.data import Preprocessor, RandomZData, TGANDataFlow
from modules.datgan_postLSTM.trainer import GANTrainer
            ...
            for col_id, col in enumerate(self.metadata['details'].keys()):
                ...
                if len(in_edges[col]) <= 1:
                    ...
                    for name in in_edges[col]:
                        ...
                        # Compute the previous states
                        ancestors = nx.ancestors(self.dag, name)
                        ancestors.add(name)
                        prev_states = []
                        for n in self.dag.nodes:
                            if n in ancestors:
