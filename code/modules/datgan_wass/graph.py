...
class GraphBuilder(ModelDescBase):
    ...
    def build_losses(self, vecs_pos, vecs_neg, extra_g=0, l2_norm=0.00001):
        r"""
        ...
        with tf.variable_scope('discrim'):
            discrim_real = self.discriminator(vecs_pos)
            discrim_fake = self.discriminator(vecs_neg)
                
        with tf.name_scope("GAN_loss"):
            score_real = tf.sigmoid(discrim_real)
            score_fake = tf.sigmoid(discrim_fake)
            ...
            with tf.name_scope("discrim"):
                d_loss_pos = tf.reduce_mean(
                    tf.nn.sigmoid_cross_entropy_with_logits(
                        logits=discrim_real,
                        labels=tf.ones_like(discrim_real)) * 0.7 + tf.random_uniform(
                            tf.shape(discrim_real),
                            maxval=0.3
                    ),
                    name='loss_real'
                )
                ...
                d_loss_neg = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
                    logits=discrim_fake, labels=tf.zeros_like(discrim_fake)), name='loss_fake')
            ...
            with tf.name_scope("gen"):
                g_loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(
                    logits=discrim_fake, labels=tf.ones_like(discrim_fake))) + \
                    tf.contrib.layers.apply_regularization(
                        tf.contrib.layers.l2_regularizer(l2_norm),
    ...
    def build_graph(self, *inputs):
        ...
        self.build_losses(vecs_pos, vecs_neg, extra_g=KL, l2_norm=self.l2norm)
