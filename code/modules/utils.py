...
# ==============================================================================
"""Auxiliary modules for domain adaptation related losses.
"""
import math
...
def create_summaries(end_points, prefix='', max_images=3, use_op_name=False):
  ...
  for layer_name in end_points:
    if use_op_name:
      ...
      name = layer_name
    if len(end_points[layer_name].shape) == 4:
      # if it's an actual image do not attempt to reshape it
      if end_points[layer_name].shape[-1] == 1 or end_points[
          layer_name].shape[-1] == 3:
    ...
    elif len(end_points[layer_name].shape) == 3:
      images = tf.expand_dims(end_points[layer_name], 3)
      tf.summary.image(
          '{}/{}'.format(prefix, name),
          images,
          max_outputs=max_images)
    elif len(end_points[layer_name].shape) == 2:
...
def reshape_feature_maps(features_tensor):
  ...
  """
  assert len(features_tensor.shape) == 4
  num_filters = features_tensor.shape[-1]
