...
class GraphBuilder(ModelDescBase):
    ...
    def build_graph(self, *inputs):
        ...
        for col_id, col in enumerate(self.metadata['details'].keys()):
            ...
            if col_info['type'] == 'category':
                noise_input = inputs[ptr]

                if self.training:
                    noise = tf.random_uniform(tf.shape(inputs[ptr]), minval=0, maxval=self.noise)
                    noise_input = (inputs[ptr] + noise) / tf.reduce_sum(
                        inputs[ptr] + noise, keepdims=True, axis=1)
